...
export function CityComparisonsTab() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-white">City Comparisons</h1>
        <p className="text-white/40 text-sm mt-1">
          How does a supply disruption hit differently across Indian cities?
        </p>
      </div>
        ...
        <div className="md:col-span-2 bg-white/[0.03] border border-white/5 rounded-2xl p-5 space-y-4">
          ...
          <div>
            <p className="text-white/50 text-xs uppercase tracking-widest mb-2">
              Add Cities to Compare <span className="text-white/25 normal-case">({selected.length}/6)</span>
            </p>
        ...
        <>
          <div className="bg-white/[0.03] border border-white/5 rounded-2xl p-6">
            <p className="text-white/60 text-xs uppercase tracking-widest mb-4">
              Projected 7-Day Profit by City (Hormuz Scenario)
            </p>
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={results} margin={{ top:5, right:10, bottom:5, left:0 }}>
                ...
                <Tooltip
                  ...
                  formatter={(v:number)=>[`₹${v.toLocaleString()}`,"Projected Profit"]} />
                <Bar dataKey="adjusted_profit_inr" radius={[4,4,0,0]}
                  label={{ position:"top", fill:"rgba(255,255,255,0.4)", fontSize:10, formatter:(v:number)=>`₹${(v/1000).toFixed(1)}K` }}>
                  {results.map((r, i) => (
          ...
          <div>
            ...
            <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4">
              {results.map(r => {
                ...
                return (
                  <div key={r.city} className="bg-white/[0.03] border border-white/5 rounded-2xl p-5 space-y-4">
                    <div className="flex items-start justify-between">
                      <div>
                        <div className="flex items-center gap-2">
                          <MapPin className="w-4 h-4" style={{ color: tc }} />
                          <span className="text-white font-semibold">{r.city}</span>
                        </div>
                        <p className="text-white/30 text-xs mt-0.5">{r.state}</p>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      {[
                        { label:"Price Level",  value:`${(r.price_multiplier*100).toFixed(0)}%`,          color:"text-white"       },
                        { label:"Margin Hit",   value:`${(r.adjusted_margin_change*100).toFixed(1)}%`,    color:"text-red-400"     },
                        { label:"Demand",       value:`${(r.adjusted_demand_change*100).toFixed(1)}%`,    color:"text-orange-400"  },
                        { label:"Profit/wk",   value:`₹${r.adjusted_profit_inr.toLocaleString(undefined,{maximumFractionDigits:0})}`, color:"text-emerald-400" },
                      ].map(m => (
                    ...
                    <div className="pt-1">
                      <p className="text-[10px] text-white/30 mb-1">Shock absorption</p>
                      <div className="w-full h-1.5 bg-white/10 rounded-full">
                        <div className="h-full rounded-full" style={{
                          width:`${Math.min(100, (r.shock_absorption / 1.5) * 100)}%`,
                          background: r.shock_absorption < 1 ? "#10b981" : "#ef4444"
                        }} />
                      </div>
                      <p className="text-[10px] text-white/20 mt-0.5">
                        {r.shock_absorption < 1 ? "Fast recovery (metro)" : "Slow recovery (rural)"}
                      </p>
                    </div>
                  </div>
                );
      ...
      {loading && (
        <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {selected.map(c => (
            <div key={c} className="bg-white/[0.03] border border-white/5 rounded-2xl p-5 animate-pulse h-40" />
          ))}
        </div>
      )}
    </div>
